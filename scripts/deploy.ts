/**
 * deploy.ts — Deploy NightFunToken to Midnight Preprod
 *
 * Run:
 ...
 * Prerequisites:
 *   1. npm install
 *   2. npm run compile          (requires compact compiler in PATH)
...
const ZK_CONFIG_PATH = path.resolve(import.meta.dirname, '..', 'contracts', 'managed', 'night-fun-token');
...
async function main() {
  ...
  console.log('\n🚀 Deploying NightFunToken to Midnight Preprod...\n');
  ...
  const compiled = CompiledContract
    .make('night-fun-token', ContractModule.Contract)
  ...
  const providers = {
    ...
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'night-fun-deploy', walletProvider: ctx.wallet as any }),
  ...
  const deployed = await deployContract(providers, { compiledContract: compiled, privateStateId: 'nightFunState', initialPrivateState: {} });

  console.log(`\n   ✅ NightFunToken deployed!`);
